...
<html>
  ...
  <body style="margin:0;">
    <div class="panel panel-info" style="width:1616px;margin:30px auto 0;">
      <a href="index.jsp" style="margin-left:20px;">生成文档到服务器</a>
    </div>
               ...
               $(".right > div > div > a").click(function(e){
                 ...
                 $.each(nowdata.datas,function(index1){
                   if(this.filename == filename){
                     switch(text){
                       case "任务单": link = this.url.rwd; break;
                       case "委托单": link = this.url.wtd; break;
                       case "报告": link = this.url.bg;  break;
                       case "意见": link = this.url.yj; break;
                       case "原始记录": link = this.url.ysjl; break;
                       case "回执": link = this.url.hz; break;
                     }
                     link = encodeURI(encodeURI(link));
                     window.open("downloadFile_luru?filepath="+link);
                   }
                 })
               });
               return;
